...
const QuizPage = ({
  ...
  return (
    <div>
      <h2 className="text-2xl mb-6 text-center">Page {pageNumber} of 2</h2>
      {questions
        .slice((pageNumber - 1) * 5, (pageNumber - 1) * 5 + 5)
        .map((q: { id: number; question: string; options: string[] }) => (
          <Question
            key={q.id}
            question={q.question}
            options={q.options}
      ...
      <div
        className={clsx(
          pageNumber !== 1 ? "flex justify-between" : "flex justify-end"
        )}
      >
        {pageNumber > 1 && (
          <button
